...
> Credit: the core scenario in this skill (savepoints, transaction-level wait queues, and 23ai assertion enqueues) is based on analysis originally published by **Jonathan Lewis** in ["Savepoint, funny"](https://jonathanlewis.wordpress.com/2026/06/26/savepoint-funny/) on his Oracle blog. See the [Credits](#credits) section at the end for details.
...
The savepoint/wait-queue scenario, the ITL/Uba framing, and the 23ai `CREATE ASSERTION` AN-enqueue discussion in this skill are based on analysis and commentary originally published by **Jonathan Lewis** — Oracle ACE Director and longtime Oracle Optimizer/internals specialist — in his article ["Savepoint, funny"](https://jonathanlewis.wordpress.com/2026/06/26/savepoint-funny/) on his blog, jonathanlewis.wordpress.com (June 26, 2026), including points raised in the post's comment thread.

All code examples, table/column names, and prose explanations above were independently written for this skill and are not reproduced from the original post. Readers wanting the original block-dump evidence, exact test case, and full reader discussion should consult the source article directly.
